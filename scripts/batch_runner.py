...
class BatchRunner:
    ...
    def _init_shared_tools(self) -> ToolRegistry:
            """
            Initialize shared tools (industrial-grade microservices architecture).
            Read paths directly from Config, no longer manually checking .pkl file existence.
            """
            data_config = self.config.get('data', {})
            embedding_config = self.config.get('embedding', {})
            
            # Align with AutoDL global data disk path
            chunks_file = data_config.get('chunks_file', '/root/autodl-tmp/arag_data/chunks.json')
            db_path = "/root/autodl-tmp/arag_data/arag_docs.db"
            
            tools = ToolRegistry()
    
            # Register keyword search (BM25 keeps using JSON loading)
            print("📦 Initializing Keyword Search (BM25)...")
            tools.register(KeywordSearchTool(chunks_file=chunks_file))
    
            # Register semantic search (directly connect to Zilliz Cloud + local SQLite)
            # No longer checking .pkl physical files, initialize directly
            print(f"🔗 Initializing Semantic Search (Zilliz Cloud + HNSW)...")
            ...
            # Register full-text reading tool (SQLite millisecond-level query)
            print(f"📚 Initializing Read Chunk Tool (SQLite)...")
            tools.register(ReadChunkTool(db_path=db_path))
            
            print("✅ All industrial-grade tools registered successfully!")
            return tools
